...
const AddEditAIEndpoint = ({
    ...
    const modelProviderLabel = intl.formatMessage({
        id: 'Apis.Details.Endpoints.AIEndpoints.AddEditAIEndpoint.model.provider',
        defaultMessage: 'Service Provider',
    });
    const modelVersionLabel = intl.formatMessage({
        id: 'Apis.Details.Endpoints.AIEndpoints.AddEditAIEndpoint.provider.version',
        defaultMessage: 'Provider Version',
    });
    ...
    const [selectedProviderVersion, setSelectedProviderVersion] = useState('');
        ...
        API.getLLMProviders()
            ...
            .catch((error) => {
                ...
                Alert.error(intl.formatMessage({
                    id: 'Apis.Details.Endpoints.AIEndpoints.AddEditAIEndpoint.error.loading.providers',
                    defaultMessage: 'Error loading AI service providers',
                }));
            });
    }, []);
    ...
    const getVersionsForProvider = (providerName) => {
        if (!llmProviders || !llmProviders.list || !providerName) return [];
        return llmProviders.list.filter((p) => p.name === providerName);
    };

    // Function to fetch endpoint configuration when provider changes
    const fetchProviderEndpointConfiguration = (providerId) => {
        if (providerId) {
            API.getLLMProviderEndpointConfiguration(providerId)
                .then((response) => {
                    if (response.body) {
                ...
                .catch((error) => {
                    console.error('Error loading provider endpoint configuration:', error);
                    Alert.error(intl.formatMessage({
                        id: 'Apis.Details.Endpoints.AIEndpoints.AddEditAIEndpoint.error.loading.config',
                        defaultMessage: 'Error loading provider endpoint configuration',
                    }));
                });
        }
    };
    ...
    useEffect(() => {
        if (endpointId) {
            ...
            if (isDefaultEndpoint) {
                ...
                // Set API key value and load all auth configuration from saved endpoint
                ...
                if (securityConfig) {
                    ...
                    if (securityConfig.type === 'apikey' && securityConfig.enabled) {
                        setApiKeyValue('********');
                    }
                    // Set AWS related values
                    if (securityConfig.type === 'aws' && securityConfig.enabled) {
                ...
                API.getApiEndpoint(apiObject.id, endpointId)
                    .then((response) => {
                        ...
                        // Also fetch provider config to ensure header names etc. are correct on edit
                        if (apiObject.subtypeConfiguration?.subtype === 'AIAPI') {
                            const config = JSON.parse(apiObject.subtypeConfiguration.configuration);
                            if (config.llmProviderId) {
                                fetchProviderEndpointConfiguration(config.llmProviderId);
                            }
                        }
                        ...
                        // Set API key value and load all auth configuration from saved endpoint
                        const envType = body.deploymentStage === CONSTS.DEPLOYMENT_STAGE.production
                            ? 'production' : 'sandbox';
    ...
    const handleApiKeyChange = (event) => {
        ...
        // Only clear the value if user is actually typing (not just focusing)
        if (apiKeyVal === '********') {
            // User clicked on the masked field, allow them to start typing fresh
            apiKeyVal = '';
        } else if (apiKeyVal === '') {
            // Keep as empty string, don't convert to null unless explicitly saving
            apiKeyVal = '';
        } else if (apiKeyVal.includes('********')) {
            // User is typing after the masked value, replace the whole thing
    ...
    const handleApiKeyBlur = () => {
        // If the field is empty and we had a saved API key, restore the masked value
        if (apiKeyValue === '' || apiKeyValue === null) {
            const envType = state.deploymentStage === CONSTS.DEPLOYMENT_STAGE.production 
                ? 'production' : 'sandbox';
            const securityConfig = state.endpointConfig?.endpoint_security?.[envType];
            
            // If there's a saved API key, show masked value instead of empty
            if (securityConfig?.type === 'apikey' && securityConfig?.enabled && securityConfig?.apiKeyValue) {
                setApiKeyValue('********');
                return;
            }
        }
        ...
        // Only add Bearer prefix for specific providers that require it
        // List of providers that require Bearer token authentication
        const providersRequiringBearer = ['MistralAI', 'OpenAI'];
        
        // Use selectedProvider (current UI selection)
        if (selectedProvider && providersRequiringBearer.includes(selectedProvider) &&
    ...
    useEffect(() => {
        // Restore provider information from API's subtypeConfiguration when editing
        if (apiObject.subtypeConfiguration?.subtype === 'AIAPI') {
            const config = JSON.parse(apiObject.subtypeConfiguration.configuration);
            if (config.llmProviderName && !selectedProvider) {
                setSelectedProvider(config.llmProviderName);
                if (config.llmProviderVersion) {
                    setSelectedProviderVersion(config.llmProviderVersion);
                }
                // Fetch endpoint configuration for this provider if ID exists
                if (config.llmProviderId) {
                    fetchProviderEndpointConfiguration(config.llmProviderId);
                }
            }
        } else if (!selectedProvider) {
            // Don't load default configuration - keep fields hidden until provider is selected
            setEndpointConfiguration({
                authenticationConfiguration: {
                    authenticationConfiguration: { "enabled": false, "type": null, parameters: {} }
                }
            });
        }
    }, [apiObject.subtypeConfiguration, selectedProvider]);

    // Clear auth fields when provider changes
    useEffect(() => {
        if (selectedProvider) {
            // Clear previous auth values when switching providers
            setApiKeyValue(null);
            setAccessKey(null);
            setSecretKey(null);
            setRegion(null);
    ...
    const handleOnBlurOnAWSCredentials = () => {
        ...
        // If the secret key field is empty and we had a saved secret key, restore the masked value
        if (secretKey === '' || secretKey === null) {
            const envType = state.deploymentStage === CONSTS.DEPLOYMENT_STAGE.production 
                ? 'production' : 'sandbox';
            const securityConfig = state.endpointConfig?.endpoint_security?.[envType];
            
            // If there's a saved secret key, show masked value instead of empty
            if (securityConfig?.type === 'aws' && securityConfig?.enabled && securityConfig?.secretKey) {
                setSecretKey('********');
                return;
            }
        }

        // Skip if secretKey is null, empty, or ********
    ...
    const apiKeyParamConfig = (() => {
        // Get the current deployment stage to access the correct security config
        const envType = state.deploymentStage === CONSTS.DEPLOYMENT_STAGE.production ? 'production' : 'sandbox';
        const securityConfig = state.endpointConfig?.endpoint_security?.[envType];
        
        // If we have saved security config with apiKeyIdentifier, use that
        if (securityConfig?.apiKeyIdentifier) {
            const isHeader = securityConfig.apiKeyIdentifierType === 'HEADER';
            const isQueryParam = securityConfig.apiKeyIdentifierType === 'QUERY_PARAMETER';
            
            return {
                authHeader: isHeader ? securityConfig.apiKeyIdentifier : null,
                authQueryParam: isQueryParam ? securityConfig.apiKeyIdentifier : null,
            };
        }
        
        // Otherwise fall back to provider configuration
        return {
            authHeader:
            ...
            <Grid item sm={12} md={12} lg={8}>
                ...
                <Grid item md={12}>
                    <Paper elevation={0} className={classes.root}>
                        ...
                        {/* Service Provider Selection - Only show when adding new endpoints */}
                        {!isEditing && (
                            <Grid container spacing={2} className={classes.deploymentStageSelect}>
                                <Grid item xs={6}>
                                    <FormControl fullWidth>
                                        <InputLabel id='model-provider-label'>{modelProviderLabel}</InputLabel>
                                        <Select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setSelectedProviderVersion('');

                                                // Update API with provider name immediately,
                                                // creating a fresh configuration
                                                const updatedSubtypeConfig = {
                                                    subtype: 'AIAPI',
                                                    configuration: {
                                                        llmProviderName: providerName,
                                                        llmProviderId: null,
                                                        llmProviderVersion: null,
                                                    },
                                                };
                                                updateAPI({ subtypeConfiguration: updatedSubtypeConfig }, false)
                                                    .catch((error) => {
                                ...
                                <Grid item xs={6}>
                                    <FormControl 
                                        fullWidth 
                                        disabled={!selectedProvider || isRestricted(['apim:api_create'], apiObject)}
                                    >
                                        <InputLabel id='model-version-label'>{modelVersionLabel}</InputLabel>
                                        <Select
                                            ...
                                            onChange={(e) => {
                                                const selectedVersionValue = e.target.value;
                                                setSelectedProviderVersion(selectedVersionValue);

                                                // Find the provider ID for the selected version
                                                const providers = getVersionsForProvider(selectedProvider);
                                                const selectedProviderData = providers.find(
                                                    (p) => p.apiVersion === selectedVersionValue,
                                                );
                                                if (selectedProviderData) {
                                                    // Create a fresh configuration based on local state,
                                                    // not stale props
                                                    const updatedSubtypeConfig = {
                                                        subtype: 'AIAPI',
                                                        configuration: {
                                                            llmProviderName: selectedProvider,
                                                            llmProviderId: selectedProviderData.id,
                                                            llmProviderVersion: selectedVersionValue,
                                                        },
                                                    };
                                                    updateAPI({ subtypeConfiguration: updatedSubtypeConfig }, false)
                                                        .then(() => {
                                                            // Fetch endpoint configuration for this provider
                                                            fetchProviderEndpointConfiguration(
                                                                selectedProviderData.id,
                                                            );
                                                        })
                                                        .catch((error) => {
                                                            console.error('Provider version update failed:', error);
                                                        });
                                                }
                                            }}
                                        >
                                            {getVersionsForProvider(selectedProvider).map((provider) => (
                                                <MenuItem key={provider.id} value={provider.apiVersion}>
                                                    {provider.apiVersion}
                                                </MenuItem>
                                            ))}
                                        </Select>
                        ...
                        <Grid container spacing={2}>
                            ...
                            {/* AI Endpoint Auth Fields - Show when provider is selected OR when editing 
                                existing endpoint with saved auth config */}
                            {((selectedProvider && IS_APIKEY_AUTH_ENABLED(endpointConfiguration)) || 
                             (isEditing && (() => {
                                 const envType = state.deploymentStage === CONSTS.DEPLOYMENT_STAGE.production 
                                     ? 'production' : 'sandbox';
                                 const securityConfig = state.endpointConfig?.endpoint_security?.[envType];
                                 return securityConfig?.type === 'apikey' && securityConfig?.enabled;
                             })())) && 
                             <>
                                 <Grid item xs={6}>
                                     <TextField
                            ...
                            {/* AWS SigV4 Auth Fields - Show when provider is selected OR when editing 
                                existing endpoint with saved auth config */}
                            {((selectedProvider && IS_AWS_SIGV4_AUTH_ENABLED(endpointConfiguration)) || 
                             (isEditing && (() => {
                                 const envType = state.deploymentStage === CONSTS.DEPLOYMENT_STAGE.production 
                                     ? 'production' : 'sandbox';
                                 const securityConfig = state.endpointConfig?.endpoint_security?.[envType];
                                 return securityConfig?.type === 'aws' && securityConfig?.enabled;
                             })())) && 
                             <>
                                 <Grid item xs={4}>
                                     <TextField
